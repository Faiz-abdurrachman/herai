...
OUT_MD = ROOT / "docs" / "HerAI_Maintenance_Documentation.md"
OUT_DOCX = ROOT / "docs" / "HerAI_Maintenance_Documentation.docx"
OUT_PDF = ROOT / "docs" / "HerAI_Maintenance_Documentation.pdf"


MARKDOWN = """# HerAI Fellowship 2026 - Dokumentasi Pemeliharaan, Pengembangan, Database, API, dan SPA
...
def style_document(doc):
    ...
    footer.text = "HerAI Fellowship 2026 - Maintenance Documentation"
...
def add_title_page(doc):
    ...
    p.add_run("HerAI Fellowship 2026\n").bold = True
    r = p.add_run("Dokumentasi Pemeliharaan, Pengembangan, Database, API, dan SPA")
    r.font.size = Pt(16)
    r.font.color.rgb = RGBColor(31, 77, 120)

    meta = doc.add_paragraph()
    meta.alignment = WD_ALIGN_PARAGRAPH.CENTER
    meta.add_run("Professional Engineering Handover\n").bold = True
    meta.add_run("Repository: woman-in-tech-FIXED\n")
    ...
    rows = [
        ("Audience", "Engineer, maintainer, admin teknis, panitia operasional"),
        ("Scope", "SPA, API, routing, database, GAS, deployment, maintenance"),
        ("Runtime", "Static SPA, local Node gateway, Google Apps Script, Google Sheets"),
        ("Primary Entry", "index.html, js/router.js, server.js, gas/Code.gs"),
    ]
...
def build_docx():
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# "):
            if "HerAI Fellowship 2026" not in stripped:
        ...
        if len(stripped) > 3 and stripped[0].isdigit() and ". " in stripped[:5]:
            ...
            parse_inline(p, stripped.split(". ", 1)[1])
            i += 1
            continue
...
def pdf_inline(text):
    ...
    for idx, part in enumerate(parts):
        if idx % 2 == 0:
            ...
            out.append(f'<font name="Courier" color="#1F4D78">{part}</font>')
    return "".join(out)


def add_pdf_table(story, lines, start, styles):
    headers = [cell.strip() for cell in lines[start].strip().strip("|").split("|")]
    idx = start + 2
    rows = []
    while idx < len(lines) and lines[idx].strip().startswith("|"):
    ...
    data = [[Paragraph(pdf_inline(cell), styles["TableHeader"]) for cell in headers]]
    for row in rows:
        data.append([Paragraph(pdf_inline(cell), styles["TableCell"]) for cell in row[: len(headers)]])
...
def build_pdf():
    ...
    story = [
        Paragraph("HerAI Fellowship 2026", styles["Title"]),
        Paragraph("Dokumentasi Pemeliharaan, Pengembangan, Database, API, dan SPA", styles["Subtitle"]),
        Paragraph("Professional Engineering Handover<br/>Repository: woman-in-tech-FIXED<br/>Generated: 19 Mei 2026", styles["Subtitle"]),
        PageBreak(),
    ]
    ...
    while i < len(lines):
        ...
        if not stripped:
            ...
            continue
        if stripped.startswith("|") and i + 1 < len(lines) and set(lines[i + 1].replace("|", "").replace("-", "").replace(":", "").strip()) == set():
            i = add_pdf_table(story, lines, i, styles)
            continue
        if stripped.startswith("# "):
            if "HerAI Fellowship 2026" not in stripped:
                story.append(Paragraph(pdf_inline(stripped[2:]), styles["H1"]))
            i += 1
            continue
        if stripped.startswith("## "):
            story.append(Paragraph(pdf_inline(stripped[3:]), styles["H1"]))
            i += 1
            continue
        if stripped.startswith("### "):
            story.append(Paragraph(pdf_inline(stripped[4:]), styles["H2"]))
            i += 1
            continue
        if stripped.startswith("- "):
            story.append(Paragraph("&bull; " + pdf_inline(stripped[2:]), styles["Bullet"]))
            i += 1
            continue
        if len(stripped) > 3 and stripped[0].isdigit() and ". " in stripped[:5]:
            story.append(Paragraph(pdf_inline(stripped), styles["Bullet"]))
            i += 1
            continue
        story.append(Paragraph(pdf_inline(stripped), styles["Body"]))
        i += 1
    ...
    doc = SimpleDocTemplate(
        ...
        title="HerAI Maintenance Documentation",
        author="Data Sorcerers",
    )
    doc.build(story)
